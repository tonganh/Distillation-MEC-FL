...
class MyLogger(flw.Logger):
    def log(self, server=None):
        ...
        self.output['client_to_edge_communication_cost'] = server.client_to_edge_communication_cost
        ...
        if not os.path.exists('results_mobile_2901'):
            os.mkdir('results_mobile_2901')

        # dataset = server['task']
        path_as_task = 'results_mobile_2901/{}'.format(server.option['task'])
