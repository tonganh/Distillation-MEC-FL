...
class CloudServer(BasicCloudServer):
    ...
    def iterate(self, t):
        ...
        self.edge_client_communication_cost.append(sum(all_total_transfer_size))
...
class EdgeServer(BasicEdge):
    ...
    def collect_distilled_data_from_client(self, clients: List[BasicMobileClient]):
        ...
        original_indices_probs =  [0.7 / original_size for i in range(original_size)]
        new_indices_probs = [0.3 / (new_size - original_size) for i in range(original_size, new_size)]

        delete_probs = original_indices_probs + new_indices_probs
        all_indices = [i for i in range(new_size)]
        if new_size > self.limit_volume:
            chosen_indices =  np.random.choice(np.array(all_indices), self.limit_volume,p = delete_probs)
