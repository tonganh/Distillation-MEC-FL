...
class EdgeServer(BasicEdge):
    ...
    def collect_distilled_data_from_client(self, clients: List[BasicMobileClient]):
        ...
        original_size = self.y_all.shape[0]
        # original_indices = [i for i in range(original_size)]
        ...
        # additional_indices = self.y_all.shape[0]
        new_size = self.y_all.shape[0]

        original_indices_probs = [
            0.7 / original_size for i in range(original_size)]
        new_indices_probs = [0.3 / (new_size - original_size)
                             for i in range(original_size, new_size)]

        delete_probs = original_indices_probs + new_indices_probs
        all_indices = [i for i in range(new_size)]
        if new_size > self.limit_volume:
            chosen_indices = np.random.choice(
                np.array(all_indices), self.limit_volume, p=delete_probs)
            self.X_all = self.X_all[chosen_indices]
            self.y_all = self.y_all[chosen_indices]
...
class MobileClient(BasicMobileClient):
    def __init__(self, option, name='', train_data=None, valid_data=None):
        ...
        self.distiller = Distiller(SUPPORT_SIZE=self.support_size, TARGET_BATCH_SIZE=10,
                                   itr=self.distill_iters, DATASET=self.dataset, save_path=self.distill_save_path, ipc=self.ipc)
