...
def get_var_reduc_coreset(K, n_samples, do_update=True):
    ...
    for i in range(n_samples):
        ...
        scores = (np.sum(K**2, 0)/np.diag(K)) - \
            np.diag(K)  # reduction in total trace
...
def get_class_kernel(X_train, class_indices, n_models=16, k=2, seed=0, device='cuda:0'):
    fixed_seed = (np.abs(seed) + 1) * np.array(list(range(n_models)))
    # print(fixed_seed)
    model_class = partial(ConvNet_wide, X_train.shape[1], net_norm='none', im_size=(
        X_train.shape[2], X_train.shape[3]), k=2)
    X_train_features, _ = get_random_features(
        X_train=X_train[class_indices], model_class=model_class, n_models=n_models, n_features_per_model=4096, device=device)
    return X_train_features @ X_train_features.T


def make_coreset(X_train, y_train, samples_per_class, n_classes, strategy, seed=0, device='cuda:0'):
        ...
        for c in classes:
            ...
            if strategy == 'mmd':
                ...
                if len(class_indices) > 10000:
                    class_indices = np.array(class_indices)[np.random.choice(
                        len(class_indices), 10000, replace=False)]

                K = get_class_kernel(X_train, class_indices,
                ...
                selected_indices = np.random.choice(
                    len(class_indices), samples_per_class, replace=False)
            else:
