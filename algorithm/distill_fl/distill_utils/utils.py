...
from algorithm.distill_fl.distill_utils.networks import MLP, ConvNet, LeNet, AlexNet, AlexNetBN, VGG11, VGG11BN, ResNet18, ResNet18BN_AP, ResNet18BN
...
def get_dataset(dataset, data_path):
    if dataset == 'MNIST':
        ...
        data = torch.load(os.path.join(
            data_path, 'tinyimagenet.pt'), map_location='cpu')
        ...
        dst_train = TensorDataset(
            images_train, labels_train)  # no augmentation
...
def match_loss(gw_syn, gw_real, args):
    ...
    if args.dis_metric == 'ours':
        ...
        gw_real_vec = torch.cat(gw_real_vec, dim=0)
        gw_syn_vec = torch.cat(gw_syn_vec, dim=0)
        dis = 1 - torch.sum(gw_real_vec * gw_syn_vec, dim=-1) / (torch.norm(
            gw_real_vec, dim=-1) * torch.norm(gw_syn_vec, dim=-1) + 0.000001)
...
def custom_match_loss(gw_syn, gw_real, device, dis_metric):
...
def custom_epoch(mode, dataloader, net, optimizer, criterion, aug, device, dc_aug_param):
    # print("custom_epoch")
    loss_avg, acc_avg, num_exp = 0, 0, 0
    net = net.to(device)
...
def evaluate_synset(it_eval, net, images_train, labels_train, testloader, args):
    ...
    trainloader = torch.utils.data.DataLoader(
        dst_train, batch_size=16, shuffle=True)
...
def custom_evaluate_synset(it_eval, net, images_train, labels_train, testloader, lr_net, device, epoch_eval_train, batch_train, dc_aug_param):
    ...
    dst_train = TensorDataset(images_train, labels_train)
    trainloader = torch.utils.data.DataLoader(
        dst_train, batch_size=batch_train, shuffle=True, num_workers=0)
    start = time.time()
    for ep in range(Epoch+1):
        loss_train, acc_train = custom_epoch(
            'train', trainloader, net, optimizer, criterion, True, device, dc_aug_param)
        if ep in lr_schedule:
            lr *= 0.1
            optimizer = torch.optim.SGD(
                net.parameters(), lr=lr, momentum=0.9, weight_decay=0.0005)
    ...
    time_train = time.time() - start
    loss_test, acc_test = custom_epoch(
        'test', testloader, net, optimizer, criterion, False, device, dc_aug_param)
    print('%s Evaluate_%02d: epoch = %04d train time = %d s train loss = %.6f train acc = %.4f, test acc = %.4f' % (
        get_time(), it_eval, Epoch, int(time_train), loss_train, acc_train, acc_test))
...
def get_eval_pool(eval_mode, model, model_eval):
    if eval_mode == 'M':  # multiple architectures
        model_eval_pool = ['MLP', 'ConvNet',
                           'LeNet', 'AlexNet', 'VGG11', 'ResNet18']
    elif eval_mode == 'B':  # multiple architectures with BatchNorm for DM experiments
        model_eval_pool = ['ConvNetBN', 'ConvNetASwishBN',
                           'AlexNetBN', 'VGG11BN', 'ResNet18BN']
    elif eval_mode == 'W':  # ablation study on network width
        model_eval_pool = ['ConvNetW32', 'ConvNetW64',
                           'ConvNetW128', 'ConvNetW256']
    elif eval_mode == 'D':  # ablation study on network depth
        model_eval_pool = ['ConvNetD1', 'ConvNetD2', 'ConvNetD3', 'ConvNetD4']
    elif eval_mode == 'A':  # ablation study on network activation function
        model_eval_pool = ['ConvNetAS', 'ConvNetAR',
                           'ConvNetAL', 'ConvNetASwish']
    elif eval_mode == 'P':  # ablation study on network pooling layer
        model_eval_pool = ['ConvNetNP', 'ConvNetMP', 'ConvNetAP']
    elif eval_mode == 'N':  # ablation study on network normalization layer
        model_eval_pool = ['ConvNetNN', 'ConvNetBN',
                           'ConvNetLN', 'ConvNetIN', 'ConvNetGN']
        ...
        model_eval_pool = [model[:model.index('BN')]] if 'BN' in model else [
            model]
