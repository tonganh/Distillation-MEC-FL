...
class MyLogger(flw.Logger):
    def log(self, server=None):
        ...
        print(self.temp.format("Edge and client transfer data:", self.output['edge_client_communication_cost'][-1]))
        ...
        experiment_df = pd.DataFrame(columns=['round','test_acc','test_loss','train_loss','val_loss','train_acc', 'val_acc','edge_client_communication_cost'])
